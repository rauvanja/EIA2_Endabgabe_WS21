...
class KitchenComponent extends StaticDrawableComponent {
    employees: Employee[];
    saladTarget: number[];
    cabbageTarget: number[];
    sauceTarget: number[];
    tomatoTarget: number[];
    onionTarget: number[];
    thekeTarget: number[];
    saladHolder: IngredientHolder;
    cabbageHolder: IngredientHolder;
    sauceHolder: IngredientHolder;
    tomatoHolder: IngredientHolder;
    onionHolder: IngredientHolder;
    placeHolder: StaticDrawableComponent;
    ...
    getIngredientTarget(ingredient: Ingredient) {
    ...
    getIngredientHolder(ingredient: Ingredient): IngredientHolder {
        if (ingredient instanceof Salat) {
        ...
        return new Promise(async resolve => {
            ...
            if (this.currentOrder.missingIngredients.length == 0) {
                ...
                this.eventTarget.addEventListener("idleEmployee", async (evtData) => {
                    let ing = this.currentOrder.missingIngredients.splice(0, 1);
                    if (ing.length) {
                        await this.addIngredientToOrder((<any>evtData).detail, ing[0]);
                    }
                    await this.waitForEmployeesToBecomeIdle();
                    resolve(true);
                });
...
class Kitchen extends KitchenComponent {
    ...
    constructor(color: string, startX: number, startY: number, width: number, height: number, theke: Theke) {
