namespace Döner {
    ...
    function setCustomerHappyNess(val: number) {
    ...
    function setEmployeeHappiness(val: number) {
        val = Math.round(val * 100);
        let asStr = val + "%";
        // @ts-ignore
    ...
    // @ts-ignore
    function setOrderType(e: any) {
        orderType = e.target.value == "yufka" ? OrderType.yufka : OrderType.döner;
    }


    let tomatoChecked: boolean;
    let sauceChecked: boolean;
    let cabbageChecked: boolean;
    let saladChecked: boolean;
    let customerTimeout: number;
    let onionChecked: boolean;
    ...
    // @ts-ignore
    function handleButtonMyClick(): void {
